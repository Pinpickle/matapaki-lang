...
test('Moving one record to another location in state', async t => {
  ...
  t.deepEqual(
    await contract.methods.get_x().call({ from: coinbase, gas: 1000000 }),
    '5',
  );

  t.deepEqual(
    await contract.methods.get_y().call({ from: coinbase, gas: 1000000 }),
    '6',
  );

  await contract.methods.move_x_to_y().send({ from: coinbase, gas: 10000000 });
