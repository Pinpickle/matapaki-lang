...
let bytecode_as_string instructions =
  concat "" (List.map (fun inst -> concat "" (List.map (Printf.sprintf "%02x") inst)) instructions)
...
let compile ast = 
  bytecode_as_string (List.map instruction_to_int_list (Codegen.instructions_of_program ast))
